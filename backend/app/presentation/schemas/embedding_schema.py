...
class EmbeddingJobResponse(BaseModel):
    ...
    status: EmbeddingStatus = Field(description="Trạng thái job: PENDING / RUNNING / COMPLETED / FAILED / RETRYING / CANCELLED")
    ...
    started_at: datetime | None = Field(description="Thời điểm job bắt đầu chạy (UTC) — null nếu chưa start")
    completed_at: datetime | None = Field(description="Thời điểm job hoàn thành (UTC) — null nếu đang chạy")
    error_message: str | None = Field(description="Thông báo lỗi nếu job kết thúc với trạng thái FAILED")
    created_at: datetime = Field(description="Thời điểm job được tạo (UTC)")


class EmbeddingStatusResponse(BaseModel):
    document_id: UUID = Field(description="UUID tài liệu")
    job_id: UUID | None = Field(description="UUID job embedding mới nhất — null nếu chưa có job nào")
    status: EmbeddingStatus | None = Field(description="Trạng thái job mới nhất — null nếu chưa có job")
