...
class ChunkResponse(BaseModel):
    ...
    page_number: int | None = Field(
        description="Số trang trong tài liệu gốc (null nếu không xác định)"
    )
    ...
    token_count: int | None = Field(
        description="Số token ước tính (dùng để kiểm soát context window)"
    )
...
class DocumentRelationResponse(BaseModel):
    ...
    relation_type: RelationType = Field(
        description="Loại quan hệ: amends, supersedes, references, implements..."
    )
...
class ProcessingStatusResponse(BaseModel):
    ...
    status: IngestionStatus = Field(
        description="Trạng thái pipeline: PENDING / RUNNING / COMPLETED / FAILED"
    )
    current_stage: str | None = Field(
        description=(
            "Giai đoạn đang chạy: extract / chunk / detect_relations"
            " (null nếu chưa bắt đầu hoặc đã xong)"
        )
    )
    started_at: datetime = Field(description="Thời điểm pipeline bắt đầu chạy (UTC)")
    completed_at: datetime | None = Field(
        description="Thời điểm pipeline hoàn thành — null nếu đang chạy"
    )
    error_message: str | None = Field(description="Thông báo lỗi nếu pipeline thất bại")
    stage_results: dict[str, Any] = Field(
        description="Kết quả chi tiết từng giai đoạn (số chunk, số quan hệ...)"
    )
