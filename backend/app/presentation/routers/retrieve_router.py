...
from app.presentation.schemas.retrieve_schema import (
    CitationResponse,
    ConflictResponse,
    ContextChunk,
...
from app.presentation.schemas.search_schema import SearchResultItem
...
def _get_ki_service() -> KnowledgeIntelligenceService:  # pragma: no cover
    raise NotImplementedError


SearchServiceDep = Annotated[SearchService, Depends(_get_search_service)]
KIServiceDep = Annotated[KnowledgeIntelligenceService, Depends(_get_ki_service)]
...
@router.post(
    ...
    description=(
        ...
        "→ Context Assembly.\n\n"
        "Kết quả là ContextPackage sẵn sàng cho Wave 4 (Prompt Builder + LLM)."
    ),
)
...
@router.post(
    ...
    description=(
        "Giống `/retrieve/context` nhưng trả thêm `raw_retrieval`: "
        "kết quả hybrid search trước khi đi qua KI pipeline. "
        "Dùng để debug pipeline, calibrate trọng số và đánh giá chất lượng ranking."
    ),
)
...
def _package_to_response(pkg: ContextPackage) -> RetrieveContextResponse:
