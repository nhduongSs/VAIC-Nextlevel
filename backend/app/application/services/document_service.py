...
from app.application.base_service import BaseService
...
class DocumentService(BaseService):
    ...
    async def upload_document(
        ...
        logger.info("document_uploaded", document_id=str(created.id), filename=original_filename)
        return created
    ...
    async def update_document(self, document_id: UUID, dto: UpdateDocumentDTO) -> Document:
        document = await self._repo.get_by_id(document_id)
        if document is None or document.is_deleted:
            raise NotFoundException(f"Document '{document_id}' not found")
        ...
        logger.info("document_updated", document_id=str(updated.id), version=updated.version)
        return updated
    ...
    async def delete_document(self, document_id: UUID) -> None:
        ...
        # best-effort: remove from storage but do not fail the request if missing
        try:
