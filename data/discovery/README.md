...
Đọc `catalog.json` trước — index phẳng, máy đọc được, liệt kê toàn bộ 51 tài liệu (17 văn bản
...
**Schema C** (1 file — `thong_tu/28_VBHN-NHNN_2023.json`, văn bản hợp nhất): tự thiết kế, không
theo A/B vì nguồn không phải vbpl.vn API (site vbpl.vn đã đổi sang Next.js app, ItemID cũ từng
dùng cho 15 file kia không còn truy cập được nữa tại thời điểm crawl file này; thuvienphapluat.vn
cũng chặn bằng Cloudflare) — phải lấy qua mirror `luatvietnam.vn` và tự strip HTML. Nội dung ở
field **`content`** (plain text đã làm sạch, không phải HTML). Có thêm `scope_note` cảnh báo
quan trọng: đây là **biểu phí B2B** (NHNN thu phí tổ chức tín dụng dùng hệ thống thanh toán liên
ngân hàng), **không phải** biểu phí ngân hàng thương mại thu khách hàng — đừng nhầm với các file
`bieu_phi` trong `bank_docs/`.

`catalog.json` đã chuẩn hoá `doc_number`/`title`/`eff_status` cho cả 3 schema.

⚠️ **Đã sửa 2026-07-18:** `catalog.json` từng có `content_path`/`relations_path` với tiền tố
`output/...` sai (còn sót lại từ trước khi thư mục được đổi tên `output/` → `data/discovery/` ở
PR #4) — toàn bộ 50 entry cũ bị gãy path nếu ai đó thử mở file theo đúng field ghi trong catalog.
Đã sửa lại tất cả thành đường dẫn tương đối đúng (relative to `data/discovery/`) và verify cả
51 entry (50 cũ + 1 mới) đều trỏ tới file tồn tại thật.
...
| VBHN 28/2023 (biểu phí thanh toán qua NHNN) | **B2B, không phải phí khách hàng** — cân nhắc loại khỏi `data/raw/` như TT 48/2014, 48/2025; giữ trong discovery làm ngữ cảnh nền nếu cần giải thích chi phí vận hành thanh toán liên ngân hàng |
